...
MYSQL_VERSIONS=5.6.49 5.7.31 8.0.21
MARIADB_VERSION=3.1.9
	...
	for m in ${M_VERSIONS[@]}
        do
			...
			docker run -it --rm \
                        ...
                        if [ -n $mysqli ]; then
                               ./sapi/cli/php ${TRAVIS_BUILD_DIR}/run-tests.php ${TRAVIS_BUILD_DIR}/ext/mysqli/tests/*phpt
                        fi
                        if [ -n $mysqlpdo ]; then
                               ./sapi/cli/php ${TRAVIS_BUILD_DIR}/run-tests.php ${TRAVIS_BUILD_DIR}/ext/pdo_mysql/tests/*phpt
                        fi
                        docker kill ${c/:/_} 
                        sleep 3
                done
